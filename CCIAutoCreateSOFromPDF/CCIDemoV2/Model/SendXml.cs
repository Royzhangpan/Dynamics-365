...
namespace CCIAutoCreateSOFromPDF.Model
{
    class SendXml
    {
        public static string invoke(string _url, string _xml)
        {
            ...
            /*request.AddQueryParameter("api-version", req.Query["api-version"]);
            request.AddQueryParameter("sp", req.Query["sp"]); 
            request.AddQueryParameter("sv", req.Query["sv"]);
            request.AddQueryParameter("sig", req.Query["sig"]);*/
