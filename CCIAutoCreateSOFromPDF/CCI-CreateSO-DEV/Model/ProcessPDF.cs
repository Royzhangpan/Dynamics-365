        ...
        public static chrysanHeader processChrysan(Stream _path)
        {
            ...
            foreach (var detail in line)
            {
                ...
                if (System.Text.RegularExpressions.Regex.IsMatch(detail.Key, @"\d+:0"))
                {
                    ...
                    salesLine.itemId = salesDetail.supplierPartNo;//"YC85921-001";// salesDetail.supplierPartNo;
                    salesLine.unit = salesDetail.unitPrice.Split('/')[1].Equals("gal") ? "GA" : salesDetail.unitPrice.Split('/')[1];
                    salesLine.qty = 0;
                    lineCount++;
                    continue;
                }
            ...
            if (salesLine != null && salesLines.Count == lineCount - 1)
            {
                salesLines.Add(salesLine);
            }
            chrysanPO.SalesLines = salesLines;
            return chrysanPO;
        }
